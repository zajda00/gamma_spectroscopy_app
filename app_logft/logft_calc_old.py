...
# The optional Java bridge is imported lazily below so the local analytical code remains
# import-safe without creating a circular dependency.
...
def compute_logft_from_values(endpoint_keV: float, half_life_s: float, branch_fraction: float, Z: int, *, backend: str = 'local') -> LogftResult:
    ...
    - `backend`: optional backend selection (`'local'` or `'radiation_report'`)

    Returns LogftResult with `f_value` and `logft` (base-10) or None if not computable.
    """
    if backend == 'radiation_report':
        try:
            from .radiation_report_backend import maybe_compute_via_radiation_report
        except Exception:
            maybe_compute_via_radiation_report = None
        if maybe_compute_via_radiation_report is not None:
            backend_result = maybe_compute_via_radiation_report(
                endpoint_keV,
                half_life_s,
                branch_fraction,
                Z,
...
def compute_logft_for_parent_level(qbeta_keV: float, parent_exc_keV: float, level_e_keV: float, half_life_ms: float, branch_percent: float, daughter_Z: int, *, backend: str = 'local') -> LogftResult:
    ...
    return compute_logft_from_values(endpoint, half_life_s, branch_fraction, daughter_Z, backend=backend)
