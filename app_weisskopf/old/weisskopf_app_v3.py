...
def row_ratio_lines(row: pd.Series, n_transitions: int) -> List[str]:
    ...
    for i, j in itertools.combinations(range(1, n_transitions + 1), 2):
        ...
        lines.append(f"   {label}: I_gamma1/I_gamma2={g_obs:.6g}, ratio_W.u.={g_pred:.6g}")
        lines.append(f"   {label} po ICC: I_tot1/I_tot2={t_obs:.6g}, ratio_W.u._tot={t_pred:.6g}")
    return lines
...
def pretty_compare_report(meta: Dict[str, object], ranked: pd.DataFrame) -> str:
    ...
    lines.append("")
    preview_cols = ["multipoles_compact", "combined_score", "comparison_basis", "effective_mismatch", "prior_penalty", "max_alpha", "observed_gamma_ratio_summary", "predicted_gamma_ratio_summary"]
